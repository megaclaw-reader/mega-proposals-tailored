      ...
      if (validFirefliesEntries.length > 0 || validJustcallEntries.length > 0) {
        try {
          ...
          if (allSummaries.length > 0) {
            ...
            const analyzeTimeout = setTimeout(() => analyzeController.abort(), 55000); // 55s client timeout (Vercel function dies at 60s)
            let analyzeRes: Response;
            try {
              analyzeRes = await fetch('/api/analyze-transcript', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
              ...
              if (abortErr.name === 'AbortError') {
                console.error('Analyze transcript timed out after 55s');
                setAnalysisStatus('error');
                alert('⚠️ Transcript analysis timed out (took too long). The proposal will be created without tailored insights. You can try again — it usually works on the second attempt.');
                analyzeRes = new Response(null, { status: 408 });
              } else {
                throw abortErr;
              }
            }
            ...
            <div className="flex flex-col items-end gap-2">
              ...
              {isSubmitting && analysisStatus === 'analyzing' && (
                <div className="text-sm text-gray-500 flex items-center gap-2">
                  ...
                  <span>AI is reading the transcript — this usually takes 20–30 seconds</span>
                </div>
              )}
            </div>
          </form>
