...
// ═══ MONTHLY BILLING + SIGNING FLOW TESTS ═══
async function testMonthlyBillingFlow() {
  const section = '═══ MONTHLY BILLING + SIGNING FLOW TESTS ═══';
  console.log(`\n${section}`);
  ...
  try {
    // Test 1: monthlyBilling quarterly proposal shows "Review & Sign Agreement"
    const page1 = await browser.newPage();
    await page1.goto(`${BASE_URL}/p/iris`, { waitUntil: 'networkidle', timeout: 30000 });
    await page1.waitForSelector('text=Proposal ID', { timeout: 15000 });

    const signBtn = await page1.locator('button:has-text("Review & Sign Agreement")').count();
    const getStartedBtn = await page1.locator('button:has-text("Get Started"), a:has-text("Get Started")').count();

    if (signBtn > 0 && getStartedBtn === 0) {
      console.log('✅ monthlyBilling quarterly shows signing flow');
      passed++;
    } else {
      console.log(`❌ monthlyBilling quarterly signing flow - sign:${signBtn} getStarted:${getStartedBtn}`);
      failed++;
    }
    await page1.close();

    // Test 2: Click signing button shows email prompt
    const page2 = await browser.newPage();
    await page2.goto(`${BASE_URL}/p/iris`, { waitUntil: 'networkidle', timeout: 30000 });
    await page2.waitForSelector('text=Proposal ID', { timeout: 15000 });
    await page2.locator('button:has-text("Review & Sign Agreement")').click();
    await page2.waitForTimeout(500);

    const emailInput = await page2.locator('input[type="email"]').count();
    const continueBtn = await page2.locator('button:has-text("Continue to Agreement")').count();

    if (emailInput > 0 && continueBtn > 0) {
      console.log('✅ Signing flow shows email prompt + Continue button');
      passed++;
    } else {
      console.log(`❌ Signing flow UI - email:${emailInput} continue:${continueBtn}`);
      failed++;
    }
    await page2.close();

  } catch (err) {
    console.log(`❌ Monthly billing test error: ${err.message}`);
    failed++;
  } finally {
    await browser.close();
  }
